...
public class IndividualOreConfigHolder {
	...
	public boolean hasTrianglePlacement() {
		if(getConfigValue("overworldExpert." + configName.toLowerCase(Locale.ROOT) + ".triangle" + configName)!=null)
			return getConfigValue("overworldExpert." + configName.toLowerCase(Locale.ROOT) + ".triangle" + configName);
		else
			return false;
	}
